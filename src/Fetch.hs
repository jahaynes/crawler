...
data DownloadResult = DownloadResult !BS.ByteString ![Cookie] ![CanonicalUrl]

fetch :: (MonadFail m, MonadResource m, DownloadRequest dr)
...
fetch man crawlerSettings requestCookies downloadRequest = do
    ...
    pure $ DownloadResult content responseCookies (nub (redirects ++ [getUrl downloadRequest]))
